...
import { CacheOptions } from '../src/interfaces/cache-options';
import { CacheRecord } from '../src/decorators/cache.record.decorator';
import { CacheRecords } from '../src/decorators/cache.records.decorator';
...
export class AppService {
  ...
  getPost(id: number): Observable<Post> {
    return from(axios.get<Post>(`${this.apiUrl}/posts/${id}`).then(({ data }) => data));
  }
  ...
  getPosts(): Observable<Post[]> {
    return from(axios.get<Post[]>(`${this.apiUrl}/posts?limit=1`).then(({ data }) => data));
  }
